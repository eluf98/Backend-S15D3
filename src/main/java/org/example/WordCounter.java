...
public class WordCounter {
    ...
    // calculateWord metodu: Metindeki her kelimenin kaç kez geçtiğini sayar
    public static Map<String, Integer> calculatedWord() {
        // Kelimeleri tutacak bir Map oluşturuyoruz
        Map<String, Integer> wordsMap = new HashMap<>();

        // Metni temizliyoruz: yalnızca harfleri ve '- 'işaretini içeren karakterlere izin veriyoruz
        String cleanedText = text.replaceAll("[^\\p{L}'-]", " ").toLowerCase();

        // Kelimeleri ayırıyoruz
        String[] words = cleanedText.split("\\s+");

        // Kelimeleri sayıyoruz ve Map'e ekliyoruz
        for (String word : words) {
            if (word.length() > 0) { // boş kelimeleri atlıyoruz
